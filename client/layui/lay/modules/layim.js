...
layui.define(['jquery', 'layer', 'laytpl', 'upload', 'webupload'], function(exports){
  ...
  //添加好友/群
  LAYIM.prototype.addDataList = function(data){
    return addDataList(data), this;
  };
  ...
  //查看聊天记录
  LAYIM.prototype.showHistoryLog = function(data){
    return showHistoryLog(data), this;
  };
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
		...
		,'<span class="layui-icon layim-tool-image" id="layim-send-image" title="上传图片">&#xe60d;</span>'
		,'{{# }; }}'
		,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon layim-tool-image" id="layim-send-file" title="发送文件">&#xe61d;</span>'
        ,'{{# }; }}'
        ,'{{# if(d.base && d.base.chatLog){ }}'
        ,'<span class="layim-tool-log" layim-event="chatLog"><i class="layui-icon">&#xe60e;</i>'+IML('聊天记录')+'</span>'
        ,'{{# }; }}'
      ,'</div>'
      ...
      ,'<div class="layim-chat-bottom">'
        ,'<div class="layim-chat-send">'
          ,'{{# if(!d.base.brief){ }}'
          ,'<span class="layim-send-close" layim-event="closeThisChat">'+IML('关闭')+'</span>'
          ,'{{# } }}'
          ,'<span class="layim-send-btn" layim-event="send">'+IML('发送')+'</span>'
          ,'<span class="layim-send-set" layim-event="setSend" lay-type="show"><em class="layui-edge"></em></span>'
          ,'<ul class="layui-anim layim-menu-box">'
            ,'<li {{d.local.sendHotKey !== "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend" lay-type="Enter"><i class="layui-icon">&#xe618;</i>'+IML('按Enter键发送消息')+'</li>'
            ,'<li {{d.local.sendHotKey === "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend"  lay-type="Ctrl+Enter"><i class="layui-icon">&#xe618;</i>'+IML('按Ctrl+Enter键发送消息')+'</li>'
          ,'</ul>'
        ,'</div>'
      ,'</div>'
    ,'</div>'
  ...
  //补齐数位
  var digit = layui.laytpl.digit = function(num){
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>'
    ...
    ,'{{# if(d.dingSwitch && d.mine && d.content.indexOf("img[")!=0 && d.content.indexOf("face[")!=0 && d.content.indexOf("file(")!=0 ){ }}'
    ,'<div class="layim-chat-ding" layim-event="ding" data-content="{{ layui.laytpl.content(d.content||"&nbsp") }}" data-username="{{ d.username }}"><i></i>'+IML('钉一下')+'</div>'
    ,'{{# } }}'
    ,'{{ layui.laytpl.content(d.content||"&nbsp") }}'
    ,'</div>'
  ,'</li>'].join('');
  
  var elemHistoryChatMain = [
    '{{# for(var i in d.data){ }}'
    ,'<li {{ d.data[i].mine ? "class=layim-chat-mine" : "" }}>'
    ,'<div class="layim-chat-user"><img src="{{ d.data[i].userInfo.avatar }}"><cite>'
    ,'{{# if(d.data[i].mine){ }}'
      ,'<i>{{ layui.laytpl.date(d.data[i].sendTime) }}</i>{{ d.data[i].userInfo.username||"佚名" }}'
    ,'{{# } else { }}'
      ,'{{ d.data[i].userInfo.username||"佚名" }}<i>{{ layui.laytpl.date(d.data[i].sendTime) }}</i>'
     ,'{{# } }}'
    ,'</cite></div>'
    ,'<div class="layim-chat-text">{{ layui.laytpl.content(d.data[i].content||"&nbsp") }}</div>'
    ,'</li>'
    ,'{{# }; }}'].join('');
  
  var elemChatList = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span title="{{ d.data.name||"佚名" }}">{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';

  var dingTpl = ['<div class="layim-ding">'
    ,'<div class="layim-ding">'
    ,'<p class="allSelect"><i></i>'+IML('全选')+'</p>'
    ,'<ul>'
    ,'{{# for(var i in d.list){ }}'
    ,'<li><div class="layim-ding-avatar"><img src="{{ d.list[i].avatar }}" class="avatar-img" data="{{ d.list[i].loginname }}"><div class="mask"></div></div>{{ d.list[i].username }}</li>'
    ,'{{# }; }}'
    ,'</ul>'
    ,'</div>'
    ,'<div class="layim-ding-bottom">'
    ,'<div class="layim-ding-send">'
    ,'<p class="layim-ding-notice">'+IML("此消息发送至对方注册手机或注册邮箱")+'</p>'
    ,'<span class="layim-send-btn">'+IML('确定')+'</span>'
    ,'</div>'
    ,'</div>'
    ,'</div>'
  ].join('');
  ...
  var post = function(options, callback, tips){
    ...
    return $.ajax({
      ...
      ,success: function(res){
        //res.code == 0 ? options.callback && options.callback(res.data||{}):'';
        //res.code == 0 ? callback && callback(res.data||{})
        //  : layer.msg(res.msg || ((tips||'Error') + ': LAYIM_NOT_GET_DATA'), {
        //    time: 5000
        //  });
        res.code == 0 ? callback && callback(res.data||{}) : console.log(res.msg || ((tips||'Error') + ': LAYIM_NOT_GET_DATA'));
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
	...
	if(cache.base.uploadImage){
		layui.webupload({
			url: cache.base.uploadImage.url || ''
			,check: 'images'
			,filetypes: 'gif,jpg,jpeg,bmp,png'
			,filesize: 20
			,success: function(res){
			  if(res.code == 0){
				res.data = res.data || {};
				if(res.data.src){
					...
					//focusInsert(thatChat.textarea[0], 'file('+ (res.data.src||'') +')['+ (res.data.name||IML('下载文件')) +']');
				}else{
					layer.msg(IML(res.msg||'上传失败'));
				}
			  } else {
				layer.msg(IML(res.msg||'上传失败'));
			  }
	...
	if(cache.base.uploadFile){
		layui.webupload({
			url: cache.base.uploadFile.url || ''
			,check: 'files'
			,filesize: 20
			,success: function(res){
			  if(res.code == 0){
  ...
  var getMessage = function(data){
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      //渲染是判断是否开启钉一下功能
      data.dingSwitch = cache.base.dingSwitch;
      ul.append(laytpl(elemChatMain).render(data));
    }
    chatListMore();
  };
  ...
  var viewChatlog = function(){
    ...
    layui.each(chatlog[thatChat.data.type + thatChat.data.id], function(index, item){
      //渲染是判断是否开启钉一下功能
      item.dingSwitch = cache.base.dingSwitch;
  ...
  var showHistoryLog = function(data){
    if(!cache.base.chatLog){
      return layer.msg('未开启更多聊天记录');
    }
    layer.close(events.chatLog.index);
    events.chatLog.index = layer.open({
      type: 1
      ,maxmin: true
      ,title: IML('与 ')+ data.name +IML(' 的聊天记录')
      ,area: ['450px', '100%']
      ,shade: false
      ,offset: 'rb'
      ,skin: 'layui-box'
      ...
      ,content: '<div class="layim-chat-main" style="height:95%;"><ul></ul></div>'
    });
    $.ajax({
      url: cache.base.chatLog + '?id=' + data.id + '&uid=' + cache.mine.id
      ,type: 'get'
      ,dataType: 'json'
      ,cache: false
      ,success: function(res){
        if(res.status){
          var content = laytpl(elemHistoryChatMain).render(res);
          $('#layui-layim-chatlog').find('ul').append(content);
          $('#layui-layim-chatlog .layim-chat-main').scrollTop($('#layui-layim-chatlog').find('ul').get(0).scrollHeight);
        }else{
          layer.msg(res.msg || ((tips||'Error') + ': LAYIM_NOT_GET_DATA'), {
            time: 5000
          });
  ...
  //表情库
  var faces = function(){
    var alt = ["[smile]", "[hee]", "[haha]", "[lovely]", "[poor]", "[nose]", "[amaze]", "[shy]", "[wink]", "[shutup]", "[despise]", "[love]", "[tear]", "[laughing]", "[kiss]", "[fallill]", "[happy]", "[contemptuous]", "[yhh]", "[zhh]", "[xi]", "[shuai]", "[grievance]", "[tu]", "[yawn]", "[hug]", "[anger]", "[doubt]", "[greedy]", "[bye]", "[reflection]", "[han]", "[kun]", "[sleep]", "[money]", "[disappointment]", "[ku]", "[se]", "[hum]", "[applause]", "[yun]", "[sadness]", "[crazy]", "[blackline]", "[sinister]", "[curse]", "[hufen]", "[heart]", "[sad]", "[pighead]", "[panda]", "[rabbit]", "[ok]", "[ye]", "[good]", "[NO]", "[zuan]", "[come]", "[weak]", "[fuck]", "[what]", "[embarrassed]", "[floatCloud]", "[awesome]", "[watch]", "[mighty]", "[ultraman]", "[gift]", "[clock]", "[microphone]", "[candle]", "[cake]"], arr = {};
  ...
  var anim = 'layui-anim-up', events = {
    ...
    ,closeChat: function(othis){
      ...
      changeChat(null, 1);
      layui.each(call.closeThisChat, function(index, item){
        item && item();
      });
    }
          ...
          ,success: function(layero){
            ...
            post(members, function(res){
              layui.each(res.list, function(index, item){
                li += '<li><a><img src="'+ item.avatar +'"></a><p title="'+item.username+'">'+ item.username +'</p></li>';
              });
              layero.find('.layim-members-list').html(li);
              layui.each(call.members, function(index, item){
                item && item(res);
              });
              othis.find('.layim-chat-members').html((res.list||[]).length + IML('人'));
            });
    ...
    ,chatLog: function(othis){
		...
		events.chatLog.index = layer.open({
			type: 1
			,maxmin: true
			,title: IML('与 ')+ thatChat.data.name +IML(' 的聊天记录')
			,area: ['450px', '100%']
			,shade: false
			,offset: 'rb'
			,skin: 'layui-box'
		...
		$.ajax({
			url: cache.base.chatLog + '?id=' + thatChat.data.id + '&type=' + thatChat.data.type + '&uid=' + cache.mine.id
			,type: 'get'
			,dataType: 'json'
			,cache: false
			,success: function(res){
			if(res.status){
				...
				//  content += laytpl(elemHistoryChatMain).render(res);
				//}
    ...
    ,ding: function(othis, e){
      var username = othis.attr('data-username');
      var msg = othis.attr('data-content');
      var chatId = thisChat().data.id;
      var chatName = thisChat().data.name;
      var members = cache.base.members || {};
      members.data = $.extend(members.data, {
        id: chatId
      });
      post(members, function(res){
        var dingIndex = layer.open({
          type: 1
          ,area: ['260px', '520px']
          ,skin: 'layui-ding'
          ,title: IML('钉一下')
          ...
          ,moveType: 1
          ,shift: 2
          ,content: laytpl(dingTpl).render(res)
          ,success: function(layero){
            var sendList = [];

            layero.find('.allSelect').click(function(){
              if($(this).find('i').hasClass('hover')){
                $(this).find('i').removeClass('hover');
                layero.find('li').each(function(){
                  var username = $(this).find('.avatar-img').attr('data');
                  var Index = sendList.indexOf(username);
                  if (Index >= 0) {
                    $(this).find('.mask').removeClass('avatar-mask');
                ...
                layero.find('li').each(function(){
                  ...
                  var Index = sendList.indexOf(username);
                  if (Index == -1) {
                    $(this).find('.mask').addClass('avatar-mask');
            ...
            layero.find('.layim-send-btn').click(function(){
              if(sendList.length == 0){
                layer.msg(IML('请选择要钉的人'));
                return;
              }
              var data = {oId :chatName,msg:msg,sender :username,users :sendList.toString()};
              $.ajax({
                url: cache.base.dingURL
                ,type: 'post'
                ,data: data
                ,dataType: 'json'
                ,cache: false
